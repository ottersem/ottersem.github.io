...
일반적으로 **Adjacnecy Matirx** $\mathbf{A} \in \mathbb{R}^{\mathcal{|V|\times|V|}}$를 통해 그래프의 관계를 표현한다. 이는 행렬로 모든 노드간의 관계를 표현하는 방법이며 다음과 같이 이 행렬로 노드의 존재 여부를 표현할 수 있다.
...
이런 그래프를 **Multi-Relational Graph**라고 부른다. 이 그래프는 adjacency tensor $\mathbf{A}\in\mathbb{R}^{\mathcal{|V|\times|R|\times|V|}}$로 표현한다. 이 때 $\mathcal{R}$은 관계의 집합을 의미한다.
...
### Feature Information
그래프에는 종종 **attribute**나 **feature** 정보가 담겨있는 경우가 있다. 대부분의 경우에는 실수 행렬 $\mathbf{X}\in\mathbb{R}^{|\mathbf{V}|\times m}$로 표현되는 node-level attribute다.
...
$$
이 때, $\mathbf{Z}\in\mathbb{R}^{|\mathcal{V|\times d}}$ 는 모든 노드들의 임베딩 벡터를 담고있는 행렬이며, $\mathbf{Z}[\mathcal{v}]$는 노드 $\mathcal{v}$에 대응하는 $\mathbf{Z}$의 행이다.
...
## Loss Functions
위에서 설명한 reconstruction loss는 시간 복잡도가 $O(|\mathcal{V}|^2|\mathcal{R}|)$이기 때문에 게산이 사실상 불가능하며, 우리가 원하는 이상적인 loss의 시간 복잡도는 $O(|\mathcal{E}|)$이다. 또 다른 문제는 low-dimensional 노드 임베딩에서 adjacency tensor를 디코딩 해야한다는 것인데, 이 tensor에는 binary 값만 담겨져 있고, mean-squre error는 binary comparison과는 맞지 않는다.
...
GNN은 일반적으로 노드 feature $\mathbf{X}\in\mathbb{R}^{d\times|\mathcal{V}|}$ 집합과 함께 그래프 $\mathcal{G=(V,E)}$를 입력받아 노드 임베딩 $\mathbf{z}_u, \forall_u \in \mathcal{v}$를 생성한다.
