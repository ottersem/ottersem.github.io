...
선택된 주성분(components_train)은 이후 데이터를 이 새로운 축에 투영하는데 사용되며, 이는 저차원으로 효과적으로 축소하면서도 데이터의 핵심적인 변동성을 유지할 수 있게 해준다.
...
- n_components : 축소할 차원의 수를 지정하며, 기본값은 **클래스의 수 - 1**중 더 작은 값으로 설정된다.
